/*
    ...
    * Assigning variables is done using 'is' Command
        * e.g. <Var> is <val>
            * where Var is unbound (unassigned) and val is bound (created (if a variable) / immediately computable)
...
isZeroBest(X) :-
    X is 0 % providing X is assigned to, will return True. if it isn't, X is then ASSIGNED 0
