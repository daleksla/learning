/*
    ...
    * Each prolog program contains a series of terms (basically rules) (be they singular or combinations of multiple terms), each ending with a fullstop (ie. '.')
        * The most basic terms has the syntax <property>(<object>). (ie. the object is a type of property)
        * A terms can establish multiple objects to a certain property: <property>(<object1>, ..., <objectn>).
        * Note: a terms taking differing amounts of objects need to be defined particularly:
